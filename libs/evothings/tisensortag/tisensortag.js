...
;(function()
{
	...
	*/
	...
	sensortag.LOCK_SERVICE = 'f000aa30-0451-4000-b000-00000000-0000'
	sensortag.LOCK_DATA = 'f000aa31-0451-4000-b000-00000000-0000'
	sensortag.LOCK_CONFIG = 'f000aa32-0451-4000-b000-000000000000'
	sensortag.LOCK_PERIOD = 'f000aa33-0451-4000-b000-000000000000'
	sensortag.LOCK_NOTIFICATION = '00002902-0000-1000-8000-00805f9b34fb'
	...
	sensortag.deviceIsSensorTag = function(device)
	{
		return (device != null) &&
			...
			(
				...
				device.name.indexOf('UTTTTTTTT')  > -1
			)
	...
	sensortag.createInstance = function()
	{
		...
		/**
		 * Public. Set the lock notification callback.
		 * @param fun - success callback called repeatedly: fun(data)
		 * @param interval - lock rate in milliseconds.
		 * @instance
		 * @public
		 */
		instance.lockCallback = function(fun, interval)
		{
			instance.lockFun = fun
			instance.lockConfig = 1 // on
			instance.lockInterval = interval
			instance.requiredServices.push(sensortag.LOCK_SERVICE)
		...
		instance.startScan = function(callbackFun) { 
				var self = this;
		...
		instance.irTemperaturePeriod = function(period)
		{
			// Set sensor update period.
			sensortag.IRTEMPERATURE_PERIOD;
			sensortag.IRTEMPERATURE_PERIOD && period && instance.device.writeCharacteristic(
				sensortag.IRTEMPERATURE_PERIOD,
				new Uint8Array([period / 10]),
				function() {},
				instance.errorFun
			)
			return instance
		...
		instance.irTemperatureReadPeriod = function(funct)
		{
			instance.device.readCharacteristic(
				sensortag.IRTEMPERATURE_PERIOD,
				funct,
		...
		instance.magnetometerPeriod = function(period)
		{
			// Set sensor update period.
			sensortag.MAGNETOMETER_PERIOD;
			sensortag.MAGNETOMETER_PERIOD && period && instance.device.writeCharacteristic(
				sensortag.MAGNETOMETER_PERIOD,
				new Uint8Array([period / 10]),
				function() {},
				instance.errorFun
		...
		/**
		 * Public. Turn on lock notification.
		 * @instance
		 * @public
		 */
		instance.lockOn = function()
		{
			instance.sensorOn(
				sensortag.LOCK_CONFIG,
				instance.lockConfig,
				sensortag.LOCK_PERIOD,
				instance.lockInterval,
				sensortag.LOCK_DATA,
				sensortag.LOCK_NOTIFICATION,
				instance.lockFun
			)
		...
		/**
		 * Public. Turn off lock notification.
		 * @instance
		 * @public
		 */
		instance.lockOff = function()
		{
			instance.sensorOff(sensortag.LOCK_DATA)
		...
		/**
		 * Calculate lock values from raw data.
		 * @param data - an Uint8Array.
		 * @return Object with fields: x, y, z.
		 * @instance
		 * @public
		 */
		instance.getLockValues = function(data)
		{
			// Lock values (Micro Tesla).
			var mx = evothings.util.littleEndianToInt16(data, 0) * (2000.0 / 65536.0) * -1
			var my = evothings.util.littleEndianToInt16(data, 2) * (2000.0 / 65536.0) * -1
			var mz = evothings.util.littleEndianToInt16(data, 4) * (2000.0 / 65536.0)
